...
**当前状态**: 图标配置完成，准备重新提交 HACS 收录申请

- ✅ **GitHub Release v2.1.0**: 已创建
- ✅ **仓库描述和 Topics**: 已配置
- ✅ **品牌图标支持**: 已启用自动加载
- ✅ **集成域名优化**: 更改为 symi 以匹配品牌仓库
- 🔄 **HACS 收录申请**: 准备重新提交

> **注意**: 集成已完成品牌图标配置，现在使用 `symi` 域名并自动从品牌仓库加载图标。

## 🎉 V2.1.0 重大更新（2025-09-02）

### ✨ 重要变更
- **域名重构**: 集成域名从 `ha_two_way_sync` 更改为 `symi`
- **品牌图标**: 启用品牌仓库图标自动加载
- **完整图标集**: 支持 icon.png, icon@2x.png, logo.png, logo@2x.png
- **HACS 兼容**: 完全符合 HACS 收录要求
...
1. 下载最新版本的代码
2. 解压后将整个 `custom_components/symi` 文件夹复制到你的 Home Assistant 配置目录下的 `custom_components` 文件夹中
3. 确保目录结构如下：
   ```
   config/
   └── custom_components/
