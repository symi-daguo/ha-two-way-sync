...
## 🔧 v1.2.6 关键修复（2025-01-25）

### 🚨 严重问题修复
- **🐛 修复同步失效**: 解决代码中存在两套不同的`_can_sync_now`和`_mark_sync_time`方法导致的逻辑冲突
- **⚡ 恢复同步功能**: 删除重复的简化版本方法，统一使用增强版本的同步逻辑
- **🔧 参数修复**: 修复`_handle_entity1_change`、`_handle_entity2_change`和`_instant_sync`中的方法调用，传入正确的参数
- **🛡️ 确保稳定性**: 统一所有同步逻辑，确保看门狗机制和防重复机制正常工作

### 📋 技术细节
- 删除第137-174行的重复方法定义
- 修复`_can_sync_now()`调用，传入`sync_direction`参数
- 修复`_mark_sync_time()`调用，传入`direction`参数
- 确保所有同步方法使用统一的逻辑

## 🎯 v1.2.5 实时跟随版（2025-01-25）

### 🚀 核心改进
- **⚡ 真正实时跟随**: 彻底重新设计同步机制，实现一方动作另一方立刻跟着动作
- **🎯 消除快速操作回弹**: 解决快速连续操作时的回弹问题，操作更流畅
- **🛡️ 看门狗机制**: 新增智能看门狗，检测到设备持续变化超过10秒时自动停止同步，彻底防止死循环
- **🔧 简化同步逻辑**: 移除复杂的抖动检测和状态历史跟踪，只关注最终状态同步
- **⏱️ 优化频率控制**: 改为简单的频率限制（1秒间隔），减少延迟但确保系统稳定

### 🛠️ 技术优化
- 移除复杂的防抖动机制，改为直接的状态变化检测
- 简化防重复机制，从2秒优化到1秒延迟清除
- 添加看门狗计数器，防止连续同步超过限制
- 优化同步锁机制，确保同步过程的原子性
- 提升状态变化检测的容差值，减少不必要的同步
