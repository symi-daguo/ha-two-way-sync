...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """设置配置条目"""
    _LOGGER.info(f"正在设置双向同步集成: {entry.title}")
    
    # 创建同步协调器
    coordinator = SimpleSyncCoordinator(
        hass=hass,
        entity1_id=entry.data["entity1"],
        entity2_id=entry.data["entity2"],
        enabled=entry.options.get("enabled", True)
    )
    ...
    try:
        ...
        _LOGGER.info(f"双向同步设置成功: {entry.title}")
    except Exception as e:
        _LOGGER.warning(f"初始设置失败，将在后台重试: {e}")
        # 不阻止集成加载，允许后续重试
    ...
    async def handle_manual_sync(call):
        ...
        if entry_id in _sync_coordinators:
            await _sync_coordinators[entry_id].manual_sync(direction)
        else:
    ...
    async def handle_reload_sync(call):
        """处理重新加载同步服务"""
        entry_id = call.data.get("config_entry_id")
        ...
        if entry_id in _sync_coordinators:
            ...
            await coordinator.async_setup()
            _LOGGER.info(f"已重新加载同步器: {entry_id}")
        else:
            _LOGGER.error(f"未找到同步器: {entry_id}")
    
    async def handle_reset_stats(call):
        """处理重置统计服务"""
        entry_id = call.data.get("config_entry_id")
        
        if entry_id in _sync_coordinators:
            coordinator = _sync_coordinators[entry_id]
            coordinator._sync_stats = {
                "total_syncs": 0,
                "successful_syncs": 0,
                "failed_syncs": 0,
                "avg_sync_time": 0.0,
            ...
            _LOGGER.info(f"已重置统计信息: {entry_id}")
        else:
            _LOGGER.error(f"未找到同步器: {entry_id}")
    ...
    if not hass.services.has_service(DOMAIN, "manual_sync"):
        hass.services.async_register(
            ...
            schema=vol.Schema({
                ...
                vol.Optional("direction", default="1to2"): cv.string,
            })
        )
    ...
    if not hass.services.has_service(DOMAIN, "reload_sync"):
        hass.services.async_register(
            DOMAIN,
            "reload_sync",
            handle_reload_sync,
    ...
    if not hass.services.has_service(DOMAIN, "reset_stats"):
        hass.services.async_register(
            DOMAIN,
            "reset_stats",
            handle_reset_stats,
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """卸载配置条目"""
    _LOGGER.info(f"正在卸载双向同步集成: {entry.title}")
    
    # 卸载平台
    unload_ok = await hass.config_entries.async_unload_platforms(entry, ["sensor", "switch"])
    
    # 获取并卸载同步器
    coordinator = _sync_coordinators.pop(entry.entry_id, None)
    if coordinator:
        await coordinator.async_unload()
        _LOGGER.info(f"双向同步卸载成功: {entry.title}")
